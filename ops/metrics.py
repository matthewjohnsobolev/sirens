        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT
                    ...
                    COUNT(*) FILTER (WHERE channel_id IS NOT NULL AND recorded_at >= NOW() - INTERVAL '24 hours' AND (source LIKE '%t.me/%' OR source LIKE '%telegram%' OR source LIKE '%api%' OR source LIKE '%ukrainealarm%')) AS auto_24h,
                    COUNT(*) FILTER (WHERE channel_id IS NOT NULL AND recorded_at >= NOW() - INTERVAL '24 hours' AND (source NOT LIKE '%t.me/%' AND source NOT LIKE '%api%' AND source NOT LIKE '%ukrainealarm%' AND (source NOT LIKE '%telegram%' OR source IS NULL))) AS manual_24h,
