...
export function telemetryKv(env: Env): KVNamespace | undefined {
  return (
    env.TELEMETRY ||
    env.STATUS_KV ||
    env.SIRENS_TELEMETRY ||
    env["sirens-telemetry"]
  );
}
...
export function healthchecksSlug(
  env: Env,
  componentKey: string,
): string | undefined {
  return componentKey === "source"
    ...
    : env.HEALTHCHECKS_ALERTS_BROADCAST_SLUG ||
        env.HEALTHCHECKS_SLUG_ALERTS_BROADCAST;
}
...
export const COMPONENTS_SPEC = [
  {
    key: "broadcast",
    name: "Телеграм-канали",
    desc: "Канали, якими ми надсилаємо вам сповіщення про тривогу.",
    source: "healthchecks",
  },
  {
    key: "source",
    name: "Джерело тривог",
    desc: "Звідки ми дізнаємось, що тривогу оголосили.",
    source: "healthchecks",
  },
  {
    key: "map",
    name: "Мапа тривог",
    desc: "Показує тривоги по всій країні в реальному часі.",
    source: "uptimerobot",
  },
  {
    key: "api",
    name: "API",
    desc: "Дані для сторонніх ботів і застосунків.",
    source: "uptimerobot",
  },
];
...
export async function fetchHealthcheckFlips(apiId: string, env: Env) {
  try {
    const res = await fetch(
      `https://healthchecks.io/api/v3/checks/${apiId}/flips/`,
      {
