...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  if (!data) {
      ...
      JSON.stringify(
        {
          status: {
            indicator: "critical",
            description:
              "Стан невідомий: сервіс моніторингу тимчасово не відповідає",
            outage_since: null,
          },
    ...
    if (!isMock)
      context.waitUntil(
        cache.put(cacheKey, errorResponse.clone()).catch(() => {}),
      );
  ...
  const isDegraded =
    data.headline.includes("— ні") || data.headline.includes("перебо");
  const isMnt =
    data.headline === "Планові роботи" || data.headline === "Технічні роботи";
  ...
  const responsePayload = {
    ...
    status: {
      ...
      outage_since:
        data.components.find((c: any) => c.outage_since)?.outage_since || null,
    },
  ...
  const cacheSeconds =
    isServiceDown || isDegraded ? ERROR_CACHE_SECONDS : JSON_CACHE_SECONDS;
