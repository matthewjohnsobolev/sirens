...
export const STATUS_WORDS: Record<string, [string, string]> = {
  ...
  major: ["збій", "збої"],
  down: ["збій", "збої"],
  mnt: ["технічні роботи", "технічні роботи"],
...
export function formatLocationLocative(
  ...
  if (customLocative) {
    return customLocative.startsWith("у ") || customLocative.startsWith("в ")
      ? customLocative
      : `у ${customLocative}`;
  }
  const cleanKey = districtKey
    ? districtKey.toLowerCase().replace(/[-_]/g, "")
    : "";
...
export function formatHourParts(
  isoDate: string,
  state: string,
  componentKey?: string,
) {
...
export function formatHourTitle(
  isoDate: string,
  state: string,
  componentKey?: string,
): string {
...
export function summarizeHours(items: any[]): string {
  ...
  for (const item of items) {
    const state =
      item.state === "major" || item.state === "minor"
        ? "down"
        : item.state || "nodata";
