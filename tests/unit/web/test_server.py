...
def test_index_markdown_content_negotiation(client):
    response = client.get("/", headers={"Accept": "text/markdown"})
    assert response.status_code == 200
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert int(response.headers["x-markdown-tokens"]) > 0
    assert response.headers["Content-Signal"] == "ai-train=yes, search=yes, ai-input=yes"
    assert "Accept" in response.headers.get("Vary", "")
    body = response.get_data(as_text=True)
    assert "title: Сирени" in body
    assert "# Сирени" in body
    assert "```json" in body


def test_index_browser_accept_defaults_to_html(client):
    response = client.get(
        "/",
...
def test_index_alternate_markdown_link(client):
    html = client.get("/").get_data(as_text=True)
    assert '<link rel="alternate" type="text/markdown" href="https://sirens.live/index.md">' in html


def test_index_direct_markdown_route(client):
    response = client.get("/index.md")
    assert response.status_code == 200
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert "title: Сирени" in response.get_data(as_text=True)


def test_issue_markdown_content_negotiation(client):
    response = client.get("/issue", headers={"Accept": "text/markdown"})
    assert response.status_code == 200
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert int(response.headers["x-markdown-tokens"]) > 0
    body = response.get_data(as_text=True)
    assert "Повідомити про збій" in body
    assert "```json" in body


def test_issue_alternate_markdown_link(client):
    html = client.get("/issue").get_data(as_text=True)
    assert '<link rel="alternate" type="text/markdown" href="https://sirens.live/issue.md">' in html


def test_issue_direct_markdown_route(client):
    response = client.get("/issue.md")
    assert response.status_code == 200
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert "Повідомити про збій" in response.get_data(as_text=True)


def test_not_found_markdown_negotiation(client):
    response = client.get("/non-existent-page", headers={"Accept": "text/markdown"})
    assert response.status_code == 404
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert "404 — Сторінку не знайдено" in response.get_data(as_text=True)


def test_server_error_markdown_negotiation(app):
    with app.test_request_context("/", headers={"Accept": "text/markdown"}):
        response = web_server.handle_server_error(Exception("test boom"))
        assert response.status_code == 500
        assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
        assert "500 — Щось зламалось у нас" in response.get_data(as_text=True)


def test_issue_post_markdown_negotiation_success(client, monkeypatch):
    monkeypatch.setattr(web_server, "_claim_report_slot", lambda: True)
    monkeypatch.setattr(web_server, "_report_to_sentry", lambda report: True)

    payload = {
        "category": "Інше",
        "message": "Чудовий сервіс",
    }
    response = client.post("/issue", data=payload, headers={"Accept": "text/markdown"})
    assert response.status_code == 200
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert "Повідомлення надіслано" in response.get_data(as_text=True)


def test_issue_post_markdown_negotiation_rate_limit(client, monkeypatch):
    monkeypatch.setattr(web_server, "_claim_report_slot", lambda: False)

    response = client.post("/issue", data={}, headers={"Accept": "text/markdown"})
    assert response.status_code == 429
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert "429 — Забагато повідомлень" in response.get_data(as_text=True)


def test_issue_post_markdown_negotiation_validation_error(client, monkeypatch):
    monkeypatch.setattr(web_server, "_claim_report_slot", lambda: True)
    ...
    assert response.status_code == 400
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert "Помилка валідації" in response.get_data(as_text=True)


def test_issue_post_markdown_negotiation_sentry_failure(client, monkeypatch):
    monkeypatch.setattr(web_server, "_claim_report_slot", lambda: True)
    monkeypatch.setattr(web_server, "_report_to_sentry", lambda report: False)
    ...
    assert response.status_code == 503
    assert response.headers["Content-Type"] == "text/markdown; charset=utf-8"
    assert "503 — Сервіс тимчасово недоступний" in response.get_data(as_text=True)


def test_caching_headers_vary_accept_on_negotiable_routes(client):
    for path in ("/", "/issue", "/index.md", "/issue.md"):
        response = client.get(path)
        assert "Accept" in response.headers.get("Vary", ""), f"Vary Accept missing for {path}"


def test_caching_headers_appends_accept_to_existing_vary(app):
    with app.test_request_context("/"):
        from flask import Response

        resp = Response("<h1>Hello</h1>", mimetype="text/html")
        resp.headers["Vary"] = "Cookie"
        updated = web_server.add_caching_headers(resp)
        assert updated.headers.get("Vary") == "Cookie, Accept"


def test_index_link_header_present_on_html(client):
    response = client.get("/")
    assert response.status_code == 200
    link = response.headers.get("Link", "")
    assert '</.well-known/api-catalog>; rel="api-catalog"' in link
    assert '</index.md>; rel="alternate"; type="text/markdown"' in link
    assert '</index.md>; rel="service-doc"' in link


def test_index_link_header_present_on_head(client):
...
def test_index_link_header_present_on_markdown(client):
    response = client.get("/", headers={"Accept": "text/markdown"})
    assert response.status_code == 200
    link = response.headers.get("Link", "")
    assert '</.well-known/api-catalog>; rel="api-catalog"' in link
...
def test_index_md_direct_route_link_header(client):
    response = client.get("/index.md")
    assert response.status_code == 200
    link = response.headers.get("Link", "")
    assert '</.well-known/api-catalog>; rel="api-catalog"' in link
...
def test_issue_link_header(client):
    for path in ("/issue", "/issue.md"):
        response = client.get(path)
        assert response.status_code == 200
        link = response.headers.get("Link", "")
        assert '</issue.md>; rel="alternate"; type="text/markdown"' in link


def test_api_catalog_route(client):
    ...
    assert (
        response.headers.get("Link") == '<https://www.rfc-editor.org/info/rfc9727>; rel="profile"'
    )

    payload = json.loads(response.get_data(as_text=True))
    assert "linkset" in payload
    entries = payload["linkset"]
    assert len(entries) >= 2
    anchors = [e["anchor"] for e in entries]
    assert "https://sirens.live/api" in anchors
    assert "https://sirens.live/issue" in anchors


def test_api_catalog_head_request(client):
...
def test_add_link_headers_appends_to_existing_link_header(app):
    from flask import Response

    with app.test_request_context("/"):
        resp = Response("<h1>Sirens</h1>", mimetype="text/html")
        resp.headers["Link"] = '<https://example.com/custom>; rel="custom"'
        updated = web_server.add_link_headers(resp)
        assert '<https://example.com/custom>; rel="custom"' in updated.headers.get("Link")
        assert '</.well-known/api-catalog>; rel="api-catalog"' in updated.headers.get("Link")

    with app.test_request_context("/issue"):
        resp = Response("<h1>Issue</h1>", mimetype="text/html")
        resp.headers["Link"] = '<https://example.com/custom>; rel="custom"'
        updated = web_server.add_link_headers(resp)
        assert '<https://example.com/custom>; rel="custom"' in updated.headers.get("Link")
        assert '</issue.md>; rel="alternate"; type="text/markdown"' in updated.headers.get("Link")

    with app.test_request_context("/.well-known/api-catalog"):
        resp = Response("{}", mimetype="application/linkset+json")
        resp.headers["Link"] = '<https://example.com/custom>; rel="custom"'
        updated = web_server.add_link_headers(resp)
        assert '<https://example.com/custom>; rel="custom"' in updated.headers.get("Link")
        assert '<https://www.rfc-editor.org/info/rfc9727>; rel="profile"' in updated.headers.get(
            "Link"
        )


def test_add_link_headers_ignores_error_responses(app):
    from flask import Response

    with app.test_request_context("/"):
        resp = Response("Error", status=500, mimetype="text/html")
        updated = web_server.add_link_headers(resp)
        assert "Link" not in updated.headers
