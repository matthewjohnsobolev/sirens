...
async function collectHealthchecks(
  specs: typeof COMPONENTS_SPEC,
  env: Env,
): Promise<(readonly [string, Probe])[]> {
  const checksList = await fetchHealthchecks(env);

  if (!checksList)
    return specs.map((spec) => [spec.key, unreachable()] as const);
  ...
  return Promise.all(
    specs.map(async (spec) => {
      ...
      let found = null;
      if (override) {
        found = checksList.find(
          (c: any) => c.slug?.toLowerCase() === override.toLowerCase(),
        );
      }
      if (!found) {
        found = checksList.find(
...
async function collectUptimeRobot(
  specs: typeof COMPONENTS_SPEC,
  env: Env,
): Promise<(readonly [string, Probe])[]> {
...
export async function computeStatusData(env: Env) {
  ...
  const rawWindows = (maintenance?.windows ||
    telemetry?.maintenance?.windows ||
    []) as MaintenanceWindowData[];
  ...
  const firstHourStart = new Date(
    currentHourStart.getTime() - (WINDOW_HOURS - 1) * 3600 * 1000,
  );
  ...
  const [hcProbes, urProbes] = await Promise.all([
    collectHealthchecks(
      COMPONENTS_SPEC.filter((c) => c.source === "healthchecks"),
      env,
    ),
    collectUptimeRobot(
      COMPONENTS_SPEC.filter((c) => c.source === "uptimerobot"),
      env,
    ),
  ]);
  ...
  for (const spec of COMPONENTS_SPEC) {
    ...
    if (probe.present) {
      ...
      else if (probe.flips && probe.flips.length > 0)
        startCandidates.push(probe.flips[0].timestamp);

      if (startCandidates.length > 0) {
        const earliestObserved = new Date(
          Math.min(...startCandidates.map((d) => d.getTime())),
        );
        if (configStart) {
          historyStart = new Date(
            Math.max(configStart.getTime(), earliestObserved.getTime()),
          );
        } else {
    ...
    const historyKnown =
      probe.present && probe.flips_ok && historyStart !== null;
    let windowStart = now;
    let intervals: { start: Date; end: Date }[] = [];

    if (historyKnown) {
      windowStart = new Date(
        Math.max(historyStart!.getTime(), firstHourStart.getTime()),
      );
      const initialUp = stateAt(
        probe.flips,
        windowStart,
        probe.live !== "down",
      );
    ...
    for (let i = 0; i < WINDOW_HOURS; i++) {
      ...
      const actualStart = new Date(
        Math.max(hourStart.getTime(), windowStart.getTime()),
      );
      ...
      for (const interval of intervalStates) {
        ...
        for (const mnt of mntIntervals) {
          const subStart = new Date(
            Math.max(actualStart.getTime(), interval.start.getTime()),
          );
          const subEnd = new Date(
            Math.min(actualEnd.getTime(), interval.end.getTime()),
          );
    ...
    if (probe.live && hours.length > 0) {
      ...
      if (probe.live === "mnt" || computed === "mnt") {
        ...
        finalState =
          probe.live === "major" || probe.live === "minor"
            ? "down"
            : probe.live;
      }
      hours[hours.length - 1].state = finalState;
      const updatedParts = formatHourParts(
        hours[hours.length - 1].date,
        finalState,
        spec.key,
      );
    ...
    const uptimePct =
      trackedSeconds > 0
        ? Math.max(
            ...
            Math.min(
              100,
              ((trackedSeconds - downSeconds) / trackedSeconds) * 100,
            ),
          )
        : null;
    ...
    if (probe.live !== "ok" && probe.live !== "mnt") {
      if (intervals.length > 0 && intervals[intervals.length - 1].end >= now) {
        ...
        if (
          probe.flips.length > 0 &&
          probe.flips[probe.flips.length - 1].up === 0
        ) {
          outageSince =
    ...
    components.push({
      ...
      monitored:
        probe.present ||
        Boolean(activeMaintenance) ||
        compMntWindows.length > 0 ||
        (spec.key === "source" && Boolean(lastSyncStr)),
      state: compState,
  ...
  const isFail = (c: any) =>
    Boolean(
      c &&
      c.monitored &&
      (c.state === "down" || c.state === "major" || c.state === "minor"),
    );
  ...
  const hasMaintenance =
    Boolean(activeMaintenance) || monitored.some((c) => c.state === "mnt");
  ...
  if (hasMaintenance) {
    ...
    subtitle =
      activeMaintenance?.subtitle || "Тривають планові технічні роботи.";
  } else if (
    !monitored.length ||
    monitored.every((c) => c.state === "nodata")
  ) {
    ...
    subtitle =
      "Дані не оновлюються, сповіщення не надсилаються. Не покладайтеся зараз на нашу систему. Використовуйте офіційний застосунок «Повітряна тривога».";
  } else if (!telegramAlive && webAlive) {
    headline = "Збій Telegram-каналів";
    subtitle =
      "Дані оновлюються, але сповіщення не надсилаються. Не покладайтеся зараз на наші канали. Використовуйте офіційний застосунок «Повітряна тривога».";
  } else if (telegramAlive && !webAlive) {
    headline = "Збій мапи тривог";
    subtitle =
      "Сповіщення в Telegram надсилаються без перебоїв, але мапа не відкривається або показує застарілі дані. Використовуйте офіційну мапу [map.ukrainealarm.com](http://map.ukrainealarm.com/).";
  } else {
    ...
    if (lastAlertDt && !isNaN(lastAlertDt.getTime())) {
      ...
      const mm = p.minute.toString().padStart(2, "0");
      const locPhrase =
        telemetry?.last_alert?.locative ||
        formatLocationLocative(
          telemetry?.last_alert?.district,
          telemetry?.last_alert?.city ||
            telemetry?.last_alert?.city_name ||
            telemetry?.last_alert?.district_name,
          (telemetry?.last_alert as any)?.location_title,
        );
      const locSuffix = locPhrase ? ` ${locPhrase}` : "";
      const timePrefix =
        dateStr && dateStr !== "сьогодні"
          ? `${dateStr} о ${hh}:${mm}`
          : `о ${hh}:${mm}`;
