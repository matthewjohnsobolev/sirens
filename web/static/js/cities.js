function setMarkerStyle(cityAlert, cityExplosion, cityShelling) {
    var markerStyle;
    if (cityExplosion === 1) {
        markerStyle = redChannelIcon;
    } else if (cityAlert === 1) {
        markerStyle = orangeChannelIcon;
    } else if (cityShelling === 1) {
        markerStyle = yellowChannelIcon; 
    } else {
        markerStyle = greenChannelIcon;
    }
    return markerStyle;
}

function setPopupContent(cityName, channelURL, channelUsername) {
    var content = `<div class='channel-popup-name'>${cityName}</div><a href="${channelURL}" class='oblast-button-link'><button class='channel-popup-button'><div class='icon-container-marker'><img class='icon-marker' src='static/img/icons/telegram.svg'></div>Підпишіться на канал, щоб отримувати сповіщення про тривогу</div></button></a>`;
    return content;
}
...
function setPopupContent2(cityName, channelURL, channelUsername, cityShelling, cityShellingSource, cityShellingTime) {
  ...
  // Добавляем кнопку для подписки на телеграм-канал
  ...
  // Если есть угроза артобстрела, добавляем соответствующий блок
  if (cityShelling === 1) {
    content += `<div class="info-block">
...
function createKyivMarkerPopup(kyivData) {
  ...
  // Добавляем блок с телеграм-каналом в начале
  popupContent += `
  ...
  } else if (kyivData.alert.status === 0) {
      popupContent += `
          <div class="info-block">
              <a href='${kyivData.alert.source}' class='oblast-button-link'>
  ...
  // Блок взрывов
  if (kyivData.explosion.status === 1) {
      popupContent += `
          <div class="info-block">
  ...
  // Блок артобстрелов
  if (kyivData.shelling && kyivData.shelling.status === 1) {
      popupContent += `
          <div class="info-block">
  ...
  // Проверяем наличие блока threats
  if (kyivData.threats) {
      // Блок ракетной угрозы
      if (kyivData.threats.missile === 1) {
          popupContent += `
              <div class="info-block">
                  <button class='red-oblast-button'>
                      <div class='icon-container'>
      ...
      // Блок разведки БПЛА
      if (kyivData.threats.reconnaissance_drone === 1) {
          popupContent += `
              <div class="info-block">
                  <button class='red-oblast-button'>
                      <div class='icon-container'>
      ...
      // Блок угрозы КАБов
      if (kyivData.threats.guided_bomb === 1) {
          popupContent += `
              <div class="info-block">
                  <button class='yellow-oblast-button'>
                      <div class='icon-container'>
      ...
      // Блок ударных БПЛА
      if (kyivData.threats.kamikaze_drone === 1) {
          popupContent += `
              <div class="info-block">
                  <button class='red-oblast-button'>
                      <div class='icon-container'>
...
fetch('/api')
  ...
  .then(data => {
        ...
        izmail.bindPopup(setPopupContent("Ізмаїл", 'tg://resolve?domain=izmail_sirens', '@izmail_alert'), customOptions); 
        ...
        zolotonosha.bindPopup(setPopupContent("Золотоноша", 'https://t.me/zolotonosha_sirens', '@zolotonosha_sirens'), customOptions); 
        ...
        zvenyhorodka.bindPopup(setPopupContent("Звенигородка", 'https://t.me/zvenyhorodka_sirens', '@zvenyhorodka_sirens'), customOptions); 
        ...
        zviahel.bindPopup(setPopupContent("Звягель", 'https://t.me/zviahel_sirens', '@izmail_alert'), customOptions); 
        ...
        korosten.bindPopup(setPopupContent("Коростень", 'https://t.me/korosten_sirens', '@zolotonosha_sirens'), customOptions); 
        ...
        berdychiv.bindPopup(setPopupContent("Бердичів", 'https://t.me/berdychiv_sirens', '@zvenyhorodka_sirens'), customOptions); 
    })
