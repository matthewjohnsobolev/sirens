...
export function getMockStatusData(
  scenario: string = "ok",
  now: Date = new Date(),
) {
  ...
  const currentHourStart = new Date(
    Math.floor(now.getTime() / (3600 * 1000)) * (3600 * 1000),
  );
  ...
  function generateComponentHours(specKey: string): {
    hours: any[];
    uptime: number | null;
    state: string;
    monitored: boolean;
    ...
    const healthyRatio =
      (totalHours - downCount - minorCount * 0.1) / totalHours;
  ...
  const components = COMPONENTS_SPEC.map((spec) => {
    ...
    return {
      ...
      state: gen.state,
      outage_since: ["down", "minor"].includes(gen.state)
  ...
  const lastAlertHour = ((nowKyiv.hour - 4 + 24) % 24)
    .toString()
    .padStart(2, "0");
  ...
  const mockLastAlertIso = new Date(
